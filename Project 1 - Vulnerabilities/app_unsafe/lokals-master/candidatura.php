...
$hst = 0;
$loca = $_REQUEST['country'];

$region = $_REQUEST['localizacao'];
$cidade = $_REQUEST['cidade'];
$postal = $_REQUEST['postal'];
$acomodacao = $_REQUEST['acomodacao'];
...
if (!empty($nome)){
  ...
  $sql = "INSERT INTO users ( password_, email, name_, cellphone, country, host) VALUES ('$pass', '$mail', '$nome', '$cell', '$loca', $hst)";
  ...
  elseif (isset($_POST['submit']) and (empty($nome) or empty($pass) or empty($mail) or empty($loca) or empty($cell))) {
    ...
    $char1 = '@';
    $char2 = '.';
    if (substr_count($mail, $char1) != 1 or substr_count($mail, $char2) != 1) {
      $error = "Email inválido!";
      echo "<script type='text/javascript'>alert('$error');</script>";
    }
    else {
      $substring1 = substr($mail, 0, substr_count($mail, $char1));
      $substring2 = substr($mail, substr_count($mail, $char1), strlen($mail));
      if (preg_match("/^\d+$/", $substring1) or preg_match("/^\d+$/", $substring2)) {
        $error = "Email inválido!";
        echo "<script type='text/javascript'>alert('$error');</script>";
      }
    }
        ...
        <nav class="navbar  navbar-expand-lg navbar-dark">
          ...
          <div class="collapse navbar-collapse text-right" id="navbarSupportedContent">
            <ul class="navbar-nav ml-auto">
              ...
              <li class="nav-item col-lg-7 active">
